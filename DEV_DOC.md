...
###     Configuration Files
        ...
        Edit `srcs/.env` and fill in all values (usernames, emails, domain name, site title). (see README.md for env example)
...
###     How Persistence Works
        ...
        The `docker-compose.yml` defines two named volumes with `driver_opts` that point to the host directories.
        When the containers start, docker mounts these directories at `/var/lib/mysql` (MariaDB) and `/var/www/html` (WordPress and NGINX).
        The init scripts in each container check if data already exists before initializing, so restarting containers does not overwrite existing data.
